...
type Level_0_tigerPOMDP <: POMDP{Int64, Tuple{Int64,Int64}, Tuple{Int64,Int64}}
    r_listen::Float64 # reward for listening (default -1)
    r_findtiger::Float64 # reward for finding the tiger (default -100)
    r_findgold::Float64 # reward for escaping (default 10)
    p_growl::Float64 # prob of correctly listening a growl(default 0.85)
...
Level_0_tigerPOMDP(;r_listen=-1.0, r_findtiger=-100.0, r_findgold=10.0, p_growl=0.85, p_reset=0.5, discount=0.95, agID=1) = Level_0_tigerPOMDP(r_listen, r_findtiger, r_findgold, p_growl, p_reset, discount,agID)

discount(p::Union{Level_l_tigerPOMDP,Level_0_tigerPOMDP}) = p.discount_factor

function isterminal(p::Union{Level_l_tigerPOMDP,Level_0_tigerPOMDP}, st::Int64)
  false
end

n_states(p::Union{Level_l_tigerPOMDP,Level_0_tigerPOMDP}) = 2
n_actions(p::Union{Level_l_tigerPOMDP,Level_0_tigerPOMDP}) = length(actions(p))
actions(::Union{Level_l_tigerPOMDP,Level_0_tigerPOMDP}) = [(OL,OL), (OL,L), (OL,OR), (L,OL), (L,L), (L,OR), (OR,OL), (OR,L), (OR,OR)]
action_index(::Union{Level_l_tigerPOMDP,Level_0_tigerPOMDP}, a::Tuple{Int64, Int64}) = (a[1]-1)*3 + a[2]
observations(::Union{Level_l_tigerPOMDP,Level_0_tigerPOMDP}) = [(GL, CL),(GL, SL),(GL, CR),(GR, CL),(GR, SL),(GR, CR)]
...
function initial_state(p::Union{Level_l_tigerPOMDP, Level_0_tigerPOMDP}, rng::AbstractRNG=MersenneTwister(1))
    rand(rng, initial_state_distribution(p))
end

function generate_s(p::Union{Level_l_tigerPOMDP, Level_0_tigerPOMDP}, s::Int64, a::Tuple{Int64,Int64}, rng::AbstractRNG)
    agID = p.agID
    oaID=1
    if agID == 1
        oaID = 2
...
#Level 0 Tiger POMDP with no information on creaks
function generate_sor(p::Level_0_tigerPOMDP, s::Int64, a::Tuple{Int64,Int64}, rng::AbstractRNG)
    agID = p.agID
    oaID=1
    if agID == 1
        oaID = 2
...
function generate_o(p::Level_0_tigerPOMDP, s::Int64, a::Tuple{Int64,Int64}, sp::Int64, rng::AbstractRNG)
    #generate o
    growl = GL
    if a[agID] == OL || a[agID] == OR
        rand(rng) < 0.5 ? growl=GL : growl=GR
...
function generate_so(p::Level_0_tigerPOMDP, s::Int64, a::Tuple{Int64,Int64}, rng::AbstractRNG)
    sp = generate_s(p,s,a,rng)
    o = generate_o(p, s, a, sp, rng)
    return sp,o
end
function reward(p::Level_0_tigerPOMDP, s::Int64, a::Tuple{Int64,Int64}, rng::AbstractRNG)
    r = 0
    if a[agID] == 2
        r = p.r_listen
    end
...
function obs_weight(p::Level_0_tigerPOMDP, s::Int64, a::Tuple{Int64,Int64}, sp::Int64, o::Tuple{Int64, Int64}, rng::AbstractRNG)
    agID = p.agID
    oaID=1
    if agID == 1
        oaID = 2
...
function initialize_static_distribution_frame_sets(::Union{Level_0_tigerPOMDP, Level_l_tigerPOMDP})
...
function initialize_intentional_frame_sets(::Union{Level_0_tigerPOMDP, Level_l_tigerPOMDP})
    lvl_l_tiger_pomdp_1 = Level_l_tigerPOMDP(agID=1)
    #lvl_0_tiger_pomdp_1 = Level_0_tigerPOMDP(agID=1)
    lvl_l_tiger_pomdp_2 = Level_l_tigerPOMDP(agID=2)
    #lvl_0_tiger_pomdp_2 = Level_0_tigerPOMDP(agID=2)
    ag_intentional_frame_set = Vector{POMDP}(1)
    ag_intentional_frame_set[1] = lvl_l_tiger_pomdp_1
    #ag_intentional_frame_set[2] = lvl_0_tiger_pomdp_1

    oa_intentional_frame_set = Vector{POMDP}(1)
    oa_intentional_frame_set[1] = lvl_l_tiger_pomdp_2
    #oa_intentional_frame_set[2] = lvl_0_tiger_pomdp_2
...
function initial_tiger_ipomdp(lvl::Int64)   #Called for agent 1 only. the others are handled internally
    #lvl_0_tiger_pomdp = Level_0_tigerPOMDP()
    ...
    if lvl > 0
        ...
        return IPOMDP_2(1,0,lvl_0_tiger_pomdp, tiger_static_dist_frame_sets, tiger_intentional_frame_sets)
    end
end
...
function Tiger_Frame_Distribution(ipomdp::IPOMDP_2, st_prob::Float64=0.5)
    p = st_prob
    ...
    if level(ipomdp) > 0
        ...
        for i in 1:length(ipomdp.oaFrames)
            push!(int_prob, (1.0/n_frames, Tiger_Frame_Distribution(ipomdp.oaFrames[i],st_prob)))
        end
        for i in 1:2
...
function initial_ipomdp_frame_distribution(pomdp::Union{Level_l_tigerPOMDP, Level_0_tigerPOMDP}, ipomdp::IPOMDP_2, st_prob::Float64 = 0.5)
    if !typeof(ipomdp.thisPOMDP) <: Union{Level_l_tigerPOMDP, Level_0_tigerPOMDP}
...
function initial_interactive_state(p::Union{Level_l_tigerPOMDP, Level_0_tigerPOMDP}, rng::AbstractRNG=Base.GLOBAL_RNG)
    rand(rng, initial_state_distribution(p))
end
